...
const features = [
  {
    number: "01",
    title: "Multi-Layer Progressive Analysis",
    description: "Three parallel research strategies — Baseline, Enhanced, and CMI Expert — each building deeper understanding. Compare quality scores, word counts, and source coverage across layers to see exactly how methodology impacts insight quality.",
    visual: "deploy",
  },
  {
    number: "02",
    title: "Intelligent Web Research",
    description: "ReAct agents autonomously search, scrape, and synthesize from the open web. SearXNG aggregates Google, Bing, DuckDuckGo, Wikipedia, and Google Scholar — with automatic fallback. Every source classified into Tier 1 (Gold), Tier 2 (Known), or Tier 3 (Unknown).",
    visual: "ai",
  },
  {
    number: "03",
    title: "Seven-Dimension Quality Scoring",
    description: "Every report is evaluated across Factual Density, Source Grounding, Analytical Depth, Specificity, Insight Quality, Completeness, and Structure Quality. Pairwise layer comparisons show concrete improvements with evidence.",
    visual: "collab",
  },
  {
    number: "04",
    title: "Source Verification & Citation Control",
    description: "60+ competing research firms automatically filtered from citations. Sources classified into credibility tiers — T1 includes IDC, Reuters, Bloomberg, SEC, WHO, and Nature. Competitor data informs analysis but is never cited in the final report.",
    visual: "security",
  },
];
  ...
  return (
    <svg viewBox="0 0 200 160" className="w-full h-full">
      ...
      {[0, 1, 2, 3, 4, 5].map((i) => {
        ...
        const radius = 50;
        const cx = Math.round(100 + Math.cos(angle) * radius);
        const cy = Math.round(80 + Math.sin(angle) * radius);
        return (
          <g key={i}>
  ...
  return (
    <svg viewBox="0 0 200 160" className="w-full h-full">
      <g>
        ...
        <text x="55" y="85" textAnchor="middle" fontSize="20" fontFamily="monospace" fill="currentColor">7</text>
      ...
      <g>
        ...
        <text x="145" y="85" textAnchor="middle" fontSize="20" fontFamily="monospace" fill="currentColor">D</text>
