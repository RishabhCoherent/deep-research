...
from research_agent.analyst.models import ResearchBoard, ResearchTrace, AnalysisResult
from research_agent.analyst.phases.decompose import decompose
from research_agent.analyst.phases.investigate import investigate
from research_agent.analyst.phases.analyze import analyze
from research_agent.analyst.phases.compose import compose
from research_agent.analyst.quality import score_research
...
async def run_analyst(
    ...
    try:
        ...
        from research_agent.analyst.phases.decompose import _fallback_framework
    ...
    # Budget: ~8 tool calls per question (think + 3 search + 2 scrape + reflect + overhead)
    # + 30 extra for tree expansion (child nodes go deeper on low-confidence questions)
    num_questions = len(framework.sub_questions)
    budget = max(80, num_questions * 8) + 30
    board = ResearchBoard(
        framework=framework,
